  ...
  useEffect(() => {
    ...
    const checkExistingProgress = async () => {
      ...
      try {
        ...
        const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/quizzes/${quizId}/progress?progressId=${cookieProgressId || ""}`)
  ...
  useEffect(() => {
    ...
    saveTimeoutRef.current = setTimeout(async () => {
      try {
        ...
        const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/quizzes/${quizId}/progress`, {
          method: "POST",
          headers: {
  ...
  const handleFinish = async (resultNodeData: any = null) => {
    try {
      ...
      if (progressId) {
        ...
        await fetch(`${process.env.NEXT_PUBLIC_API_URL}/quizzes/${quizId}/progress`, {
          method: "PATCH",
          headers: {
            "Content-Type": "application/json",
          },
      ...
      const token = localStorage.getItem("token")
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/quizzes/quiz-attempts`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
