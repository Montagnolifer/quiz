  ...
  useEffect(() => {
    ...
    const checkExistingProgress = async () => {
      ...
      try {
        ...
        const response = await fetch(`http://localhost:3000/quizzes/${quizId}/progress?progressId=${cookieProgressId || ""}`)
  ...
  useEffect(() => {
    ...
    saveTimeoutRef.current = setTimeout(async () => {
      try {
        ...
        const response = await fetch(`http://localhost:3000/quizzes/${quizId}/progress`, {
          method: "POST",
          headers: {
  ...
  const handleFinish = async (resultNodeData: any = null) => {
    try {
      ...
      if (progressId) {
        ...
        await fetch(`http://localhost:3000/quizzes/${quizId}/progress`, {
          method: "PATCH",
          headers: {
            "Content-Type": "application/json",
          },
      ...
      const token = localStorage.getItem("token")
      const response = await fetch("http://localhost:3000/quizzes/quiz-attempts", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
