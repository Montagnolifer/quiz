...
export default async function AnalyticsPage({ params }: { params: { id: string } }) {
  ...
  try {
    ...
    //const resQuiz = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/quizzes/${id}`)
    const resQuiz = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/quizzes/${id}`)
    const quiz = await resQuiz.json()
    
    //const resAttempts = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/quizzes/quiz-attempts/${id}`)
    const resAttempts = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/quizzes/quiz-attempts/${id}`)
    const attempts = await resAttempts.json()
    
    //const resAnalytics = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/quizzes/quiz-analytics/${id}`)
    const resAnalytics = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/quizzes/quiz-analytics/${id}`)

    const analytics = await resAnalytics.json()
    
    //const resProgress = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/quizzes/quiz-progress/${id}`)
    const resProgress = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/quizzes/quiz-progress/${id}`)
