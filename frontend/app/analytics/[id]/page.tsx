...
export default async function AnalyticsPage({ params }: { params: { id: string } }) {
  ...
  try {
    ...
    const resQuiz = await fetch(`http://localhost:3005/api/quizzes/${id}`)
    const quiz = await resQuiz.json()
    
    const resAttempts = await fetch(`http://localhost:3005/api/quizzes/quiz-attempts/${id}`)
    const attempts = await resAttempts.json()
    
    const resAnalytics = await fetch(`http://localhost:3005/api/quizzes/quiz-analytics/${id}`)
    const analytics = await resAnalytics.json()
    
    const resProgress = await fetch(`http://localhost:3005/api/quizzes/quiz-progress/${id}`)
